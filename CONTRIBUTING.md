...
4. The npm package publishes automatically, routed by channel like the NuGet packages: a pre-release version goes to **GitHub Packages** as `@dignite-projects/ng.vault-extract` (the scope GitHub Packages requires), a stable version goes to **npmjs** as `@dignite/ng.vault-extract`. A stable npm release additionally requires every `@dignite/*` dependency to exist on npmjs; `@dignite/ng.flex-fields` and `@dignite/ng.flex-fields-ckeditor` are published there since `10.0.0-rc.11`, which closed [#577](https://github.com/dignite-projects/vault-extract/issues/577). The **NuGet** side has its own gate that is still open: a stable `Dignite.Vault.Extract.*` depending on a pre-release `Dignite.Abp.FlexFields.*` trips `NU5104`, so stable NuGet publishing waits for flex-fields `10.0.0` — see [#558](https://github.com/dignite-projects/vault-extract/issues/558).
5. **Immediately open the next development version**: bump `<Version>` in `common.props` and the npm `version` to the next pre-release (e.g. `0.2.0-preview.2` → `0.2.0-preview.3`) in a standalone `chore(release): bump version` commit. Because the release version is read from `common.props` (not the tag), leaving it on the just-released value means accumulating `[Unreleased]` work still reports a published version and a `workflow_dispatch` build would re-emit artifacts that collide with the published package.
