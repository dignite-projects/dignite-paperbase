...
4. The npm package publishes automatically, routed by channel like the NuGet packages: a pre-release version goes to **GitHub Packages** as `@dignite-projects/vault-extract` (the scope GitHub Packages requires), a stable version goes to **npmjs** as `@dignite/vault-extract`. A stable npm release additionally requires every `@dignite/*` dependency to exist on npmjs — `@dignite/ng.flex-fields` does not yet, so stable npm publishing is gated on [#577](https://github.com/dignite-projects/vault-extract/issues/577).
