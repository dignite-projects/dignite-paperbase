    ...
    : PaperbaseApplicationTestBase<DocumentChatAppServiceTestModule>
{
    ...
    // ── 4. Global scope: null DocumentId + null DocumentTypeCode ──────────────

    [Fact]
    public async Task Should_Pass_Global_Scope_When_Both_DocumentId_And_TypeCode_Null()
    {
        ...
        captured!.DocumentId.ShouldBeNull();
        captured.DocumentTypeCode.ShouldBeNull();
    }

    // ── 5. DocumentId scope (complements test #4 in #58 which covered DocumentTypeCode only) ─

    [Fact]
    public async Task Should_Pass_DocumentId_Scope()
    {
        // IDocumentRepository is a substitute (returns null from GetAsync without throwing);
        // the AppService ignores the return value and creates the conversation successfully.
        ...
        await WithUnitOfWorkAsync(async () =>
        {
            using (ChangeUser(OwnerUserId))
            {
                await _appService.SendMessageAsync(conversationId, new SendChatMessageInput
                {
                    Message      = "document-specific question",
                    ClientTurnId = Guid.NewGuid()
                });
            }
        });
    ...
    // ── 6. TopK + MinScore overrides from conversation propagate to search ─────

    [Fact]
    public async Task Should_Pass_TopK_And_MinScore_Overrides_From_Conversation()
    {
        var conversationId = await CreateConversationAsync(topK: 17, minScore: 0.42);
    ...
    // ── 7. Multi-turn history depth: 5 turns accumulate in the LLM payload ────
