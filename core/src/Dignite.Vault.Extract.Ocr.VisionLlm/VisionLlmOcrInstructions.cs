...
internal static class VisionLlmOcrInstructions
{
    ...
    public const string SystemPrompt =
        ...
        "Render every table strictly as GitHub-Flavored Markdown: a header row, a \"|---|---|\" separator row, then one row per record, columns delimited by \"|\". Never render a table as LaTeX (\\begin{tabular}, \\hline, \\\\) or any other non-Markdown syntax, and never emit HTML comments, bounding-box coordinates, or other layout annotations — output only the transcribed content, never anything about the layout itself. " +
        ...
        "Do NOT transcribe running page headers, running page footers, or standalone page numbers — that is, boilerplate repeated at the very top or bottom edge of every page, for example a confidentiality, copyright, or filename line, or page numbering such as \"3\", \"- 3 -\", \"Page 3 of 10\", or \"第 3 页\". You see only one page at a time, so judge this by position and style, not by confirming repetition: small print set apart from the body flow and flush to the very top or bottom edge is running header/footer chrome even on a single page. " +
